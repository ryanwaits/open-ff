...
function PlayerPage() {
	...
	const myTeam = useQuery({
		...
		queryFn: () =>
			getTeam({ data: { leagueId, rosterId: Number(rosterId), week } }),
	...
	const context = mine
		? {
				label:
					mine.slot === "starter"
						? `Starting at ${mine.starterSlot}`
						: "On your bench",
				rows: [
					["Slot", mine.starterSlot ?? "Bench"] as [string, string],
					[
						"This week",
						mine.weekPts != null ? String(mine.weekPts) : "Not played",
					] as [string, string],
				],
			}
		: {
				...
				rows: [
					...
					[
						...
						league.data && rosterId != null
							? (() => {
									const row = league.data.standings.find(
