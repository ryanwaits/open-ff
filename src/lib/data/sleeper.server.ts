...
function loadPlayers(): Record<string, SlimPlayer> {
  ...
  playersById = JSON.parse(readFileSync(join(process.cwd(), "data/players-slim.json"), "utf8")) as Record<
    string,
    SlimPlayer
  >;
  return playersById;
...
function loadLeadersSeed(): StatSeed[] {
  ...
  leaders2025 = JSON.parse(readFileSync(join(process.cwd(), "data/stats-2025.json"), "utf8")) as StatSeed[];
  return leaders2025;
}
...
async function fetchSeasonStatRows(season: string): Promise<StatSeed[]> {
  const raw = await sget<Record<string, RawStat>>(`/stats/nfl/regular/${season}`, 6 * 60 * 60 * 1000);
  const rows: StatSeed[] = [];
...
export async function lookupUser(query: string): Promise<{
  ...
  const lists = await Promise.all(
    years.map(async (season) => {
      try {
        return (await sget<SleeperLeague[] | null>(`/user/${user!.user_id}/leagues/nfl/${season}`, 60_000)) ?? [];
      } catch {
        return [];
      }
    }),
...
function formatLabel(league: SleeperLeague): string {
  ...
  return [kind, `${league.settings.num_teams ?? league.total_rosters}-team`, bb].filter(Boolean).join(" · ");
}
...
export async function loadMatchups(leagueId: string, week: number): Promise<MatchupPair[]> {
  ...
  function sideOf(m: RawMatchup): MatchupSide {
    ...
    const label = teamLabel(roster?.owner_id ? byUser.get(roster.owner_id) : undefined, m.roster_id);
    ...
    const starters: StarterLine[] = startSlots.map((slot, i) => {
      ...
      return { slot: slotLabel(slot), playerId, player: getPlayer(playerId), points: pts ?? null, game: null };
    });
...
export async function loadActivity(leagueId: string, week: number): Promise<ActivityItem[]> {
  ...
  const items = (raw ?? []).map((tx) => ({
    ...
    adds: Object.keys(tx.adds ?? {}).map((id) => {
      const p = getPlayer(id);
      return { playerId: id, name: playerName(id), pos: p?.position ?? null, team: p?.team ?? null };
    }),
...
const DEF_ALIASES: Record<string, string> = {
  ...
  sf: "SF", sanfrancisco: "SF", "49ers": "SF", niners: "SF",
...
export function writeRecap(
  ...
  if (high) {
    ...
    if (stud?.player) {
      bullets.push(`${stud.player.full_name} carried ${high.teamName} with ${stud.points?.toFixed(1)} from the ${stud.slot} slot.`);
    }
  }
  ...
  const wireNote = topAdd
    ...
    : adds.length
      ...
      : null;
  return { week, leagueName, kicker: `Week ${week} dispatch`, headline, dek, bullets, box, wireNote };
}
...
async function timed<T>(fn: () => Promise<T>) {
  ...
  try {
    ...
    return { ok: false as const, ms: Date.now() - t0, error: err instanceof Error ? err.message : "failed" };
  }
}
...
export async function probeSources(): Promise<SourceStatus[]> {
  const [sleeper, espn, nflverse] = await Promise.all([
    ...
    timed(async () => {
      const res = await fetch("https://api.github.com/repos/nflverse/nflverse-data/releases/tags/stats_player", {
